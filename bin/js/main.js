...
var filters_custom_Main = function() {
	this.exampleLeft = 40;
	this.exampleTop = 40;
	...
	this.addExample("original",[]);
	this.addExample("color * 0.5",[new filters_custom_BasicFilter("\r\n\t\t\tcolor *= 0.5;\r\n\t\t")]);
	this.addExample("1.0 - color.rgb (invert)",[new filters_custom_BasicFilter("\r\n\t\t\tcolor.rgb = vec3(1.0) - color.rgb;\r\n\t\t")]);
	this.addExample("tex coords",[new filters_custom_BasicFilter("\r\n\t\t\tcolor.rg = vTextureCoord.xy;\r\n\t\t\tcolor.ba = vec2(1.0);\r\n\t\t")]);
	this.filterWithUniforms = new filters_custom_FilterWithUniforms(0.0,0.0,0.0);
	this.addExample("multiply",[this.filterWithUniforms]);
	this.addExample("filter coord",[new filters_custom_AdvancedFilter("\r\n\t\t\tcolor.rg = vFilterCoord.xy;\r\n\t\t\tcolor.ba = vec2(1.0);\r\n\t\t")]);
	this.addExample("ellipse mask",[new filters_custom_AdvancedFilter("\r\n\t\t\t// top left (-1,-1), bottom right (1,1)\r\n\t\t\tvec2 relative = 2.0 * (vFilterCoord.xy - vec2(0.5));\t\r\n\t\t\t\r\n\t\t\t// distance from the center of the sprite to processed pixel\r\n\t\t\tfloat distance = length(relative);\r\n\t\t\t\r\n\t\t\t// clamp - http://www.shaderific.com/glsl-functions/#clamp\t\t\t\r\n\t\t\tcolor.a = clamp(0.0, 1.0, 1.0 - distance);\t\t\t\t\r\n\t\t")]);
	this.addExample("displacement",[new filters_custom_AdvancedFilter("\r\n\t\t\tvec2 relative = vFilterCoord.xy - vec2(0.5);\t\r\n\t\t\t\r\n\t\t\t// calculate displacement\r\n\t\t\tfloat distance = length(relative);\r\n\t\t\trelative *= (1.0 - distance);\r\n\t\t\t\r\n\t\t\t// map back to filter coord\r\n\t\t\trelative = relative + vec2(0.5);\r\n\t\t\t\r\n\t\t\t// map to texture coord\r\n\t\t\tvec2 texCoord = filterToTexCoord(relative);\r\n\t\t\t\r\n\t\t\t// get pixel value\r\n\t\t\tcolor = texture2D(uSampler, texCoord);\r\n\t\t")]);
};
filters_custom_Main.__name__ = true;
filters_custom_Main.main = function() {
	new filters_custom_Main();
};
filters_custom_Main.__super__ = pixi_plugins_app_Application;
filters_custom_Main.prototype = $extend(pixi_plugins_app_Application.prototype,{
	update: function(_) {
		var time = window.performance.now() * 0.001;
		this.filterWithUniforms.setMultiplyRGB((Math.sin(time) + 1.0) * .5,(Math.cos(time) + 1.0) * .5,1.0);
	}
	,addExample: function(desc,filters1) {
		if(this.exampleLeft + 200 > this.renderer.width) {
			this.exampleLeft = 40;
			this.exampleTop += 100;
		}
		var text = new PIXI.Text(desc + " (on)",{ fontSize : "15px"});
		text.x = this.exampleLeft;
		text.y = this.exampleTop;
		this.stage.addChild(text);
		var g = this.stage.addChild(PIXI.Sprite.from("img/logo.png"));
		g.x = text.x;
		...
		var toggle = function(_) {
			...
			g.filters = filtersEnabled ? filters1 : null;
		};
		g.on("mousedown",toggle);
		g.on("touchstart",toggle);
		this.exampleLeft += 240;
		return g;
	}
	,__class__: filters_custom_Main
});
var filters_custom_BasicFilter = function(glsl) {
	var fragSrc = StringTools.replace("\r\n\t\tvarying vec2 vTextureCoord;\t\t// texture coordinate provided by PIXI default vertex shader\t\t\t\r\n\t\tuniform sampler2D uSampler;\t\t// texture itself provided by PIXI default uniforms\r\n\r\n\t\tvoid main()\r\n\t\t{\r\n\t\t\t// get pixel value from uSample at vTextureCoord position\r\n\t\t\t// color is 4-component vector with rgba values\r\n\t\t\tvec4 color = texture2D(uSampler, vTextureCoord);\r\n\t\t\t// custom glsl code will be injected here\r\n\t\t\t// it will modify color value in some way\r\n\t\t\t%INJECT%\r\n\t\t\t// gl_FragColor is colors \"final destination\"\r\n\t\t\tgl_FragColor = color;\r\n\t\t}\r\n\t","%INJECT%",glsl);
	PIXI.Filter.call(this,null,fragSrc,null);
};
filters_custom_BasicFilter.__name__ = true;
filters_custom_BasicFilter.__super__ = PIXI.Filter;
filters_custom_BasicFilter.prototype = $extend(PIXI.Filter.prototype,{
	__class__: filters_custom_BasicFilter
});
var filters_custom_FilterWithUniforms = function(r,g,b) {
	PIXI.Filter.call(this,null,"\r\n\t\tvarying vec2 vTextureCoord;\t\t\t\t\r\n\t\tuniform sampler2D uSampler;\t\t\r\n\t\tuniform vec3 multiply;\t//this is our Float32Array (mapped by name and type)\r\n\t\t\r\n\t\tvoid main()\r\n\t\t{\r\n\t\t\tvec4 color = texture2D(uSampler, vTextureCoord);\r\n\t\t\tcolor.rgb *= multiply;\r\n\t\t\tgl_FragColor = color;\r\n\t\t}\r\n\t",null);
	this.uniforms.multiply = haxe_io__$Float32Array_Float32Array_$Impl_$.fromArray([r,g,b]);
};
filters_custom_FilterWithUniforms.__name__ = true;
filters_custom_FilterWithUniforms.__super__ = PIXI.Filter;
filters_custom_FilterWithUniforms.prototype = $extend(PIXI.Filter.prototype,{
	setMultiplyRGB: function(r,g,b) {
		this.uniforms.multiply[0] = r;
		this.uniforms.multiply[1] = g;
		this.uniforms.multiply[2] = b;
	}
	,__class__: filters_custom_FilterWithUniforms
});
var filters_custom_AdvancedFilter = function(glsl) {
	...
	this.uniforms.frameSize = haxe_io__$Float32Array_Float32Array_$Impl_$.fromArray([0,0]);
	this.autoFit = false;
	this.padding = 0;
};
filters_custom_AdvancedFilter.__name__ = true;
filters_custom_AdvancedFilter.__super__ = PIXI.Filter;
filters_custom_AdvancedFilter.prototype = $extend(PIXI.Filter.prototype,{
	apply: function(filterManager,input,output,clear,currentState) {
		this.uniforms.frameSize[0] = input.sourceFrame.width;
		this.uniforms.frameSize[1] = input.sourceFrame.height;
		PIXI.Filter.prototype.apply.call(this,filterManager,input,output,clear,currentState);
	}
	,__class__: filters_custom_AdvancedFilter
});
...
haxe_io__$Float32Array_Float32Array_$Impl_$.__name__ = true;
...
filters_custom_Main.GRID_WIDTH = 200;
filters_custom_Main.GRID_HEIGHT = 60;
filters_custom_Main.GAP = 40;
filters_custom_BasicFilter.FRAG_SRC_TEMPLATE = "\r\n\t\tvarying vec2 vTextureCoord;\t\t// texture coordinate provided by PIXI default vertex shader\t\t\t\r\n\t\tuniform sampler2D uSampler;\t\t// texture itself provided by PIXI default uniforms\r\n\r\n\t\tvoid main()\r\n\t\t{\r\n\t\t\t// get pixel value from uSample at vTextureCoord position\r\n\t\t\t// color is 4-component vector with rgba values\r\n\t\t\tvec4 color = texture2D(uSampler, vTextureCoord);\r\n\t\t\t// custom glsl code will be injected here\r\n\t\t\t// it will modify color value in some way\r\n\t\t\t%INJECT%\r\n\t\t\t// gl_FragColor is colors \"final destination\"\r\n\t\t\tgl_FragColor = color;\r\n\t\t}\r\n\t";
filters_custom_FilterWithUniforms.FRAG_SRC = "\r\n\t\tvarying vec2 vTextureCoord;\t\t\t\t\r\n\t\tuniform sampler2D uSampler;\t\t\r\n\t\tuniform vec3 multiply;\t//this is our Float32Array (mapped by name and type)\r\n\t\t\r\n\t\tvoid main()\r\n\t\t{\r\n\t\t\tvec4 color = texture2D(uSampler, vTextureCoord);\r\n\t\t\tcolor.rgb *= multiply;\r\n\t\t\tgl_FragColor = color;\r\n\t\t}\r\n\t";
filters_custom_AdvancedFilter.VERT_SRC = "\r\n\t\tprecision highp float;\t\t//default precision is different for fragment and vertex shader, so we set one for both of them\r\n\t\t\r\n\t\tattribute vec2 aVertexPosition;\r\n\t\tattribute vec2 aTextureCoord;\r\n\t\tuniform mat3 projectionMatrix;\r\n\t\t\r\n\t\tuniform vec2 frameSize;\t\t\t//filter frame size\r\n\t\tuniform vec4 filterArea;\t\t//supplied by PIXI\r\n\t\t\r\n\t\tvarying vec2 vTextureCoord;\r\n\t\tvarying vec2 vFilterCoord;\r\n\t\tvoid main(void){\r\n\t\t   gl_Position = vec4((projectionMatrix * vec3(aVertexPosition, 1.0)).xy, 0.0, 1.0);\r\n\t\t   \r\n\t\t   //calculates filter coords\r\n\t\t   vFilterCoord = aTextureCoord * filterArea.xy / frameSize.xy;\t//filterArea.xy - width/height in pixels\r\n\t\t   \r\n\t\t   vTextureCoord = aTextureCoord;\r\n\t\t}\r\n\t";
filters_custom_AdvancedFilter.FRAG_SRC = "\r\n\t\tprecision highp float;\t//default precision is different for fragment and vertex shader, so we set one for both of them\r\n\t\t\r\n\t\tvarying vec2 vTextureCoord;\t\t\t\t\r\n\t\tvarying vec2 vFilterCoord;\t//vertex shader already calculated this by: vFilterCoord = ( filterMatrix * vec3( vTextureCoord, 1.0)  ).xy;\r\n\t\tuniform sampler2D uSampler;\t\r\n\t\t\r\n\t\tuniform vec2 frameSize;\t\t\t//filter frame size\r\n\t\tuniform vec4 filterArea;\t\t//supplied by PIXI\r\n\t\t\r\n\t\t// utility to map filterCoord back to texture coord\r\n\t\tvec2 filterToTexCoord(vec2 filterCoord)\r\n\t\t{\r\n\t\t\treturn filterCoord * frameSize.xy / filterArea.xy;\r\n\t\t}\r\n\t\t\r\n\t\tvoid main()\r\n\t\t{\r\n\t\t\tvec4 color = texture2D(uSampler, vTextureCoord);\r\n\t\t\t%INJECT%\r\n\t\t\tgl_FragColor = color;\r\n\t\t}\r\n\t";
js_Boot.__toStr = ({ }).toString;
js_html_compat_Float32Array.BYTES_PER_ELEMENT = 4;
js_html_compat_Uint8Array.BYTES_PER_ELEMENT = 1;
filters_custom_Main.main();
